...
const pesertaOptions = computed(() => {
    return props.pesertas
        .filter(p => !p.pencalonan?.some(c => props.jabatanOptions.includes(c.jabatan as any)))
        .map(p => ({
            value: p.id,
    ...
    const submissionPromise = new Promise<{ message: any }>((resolve, reject) => {
        formInertia.post("/calon", {
            ...
            onError: (errors) => {
                ...
                reject(firstError || 'Terjadi kesalahan saat menyimpan data.');
            },
        });
    });
                        ...
                        <div className="w-full">
                            <form class="w-full space-y-8" @submit.prevent="onSubmit">
                                ...
                                <div v-for="(field, index) in fields" :key="field.key"
                                    class="border rounded-lg p-4 space-y-4 relative bg-gray-50/50">

                                    <h4 class="font-medium text-sm text-gray-700">Calon #{{ index + 1 }}</h4>

                                    <Button v-if="fields.length > 1" type="button" variant="destructive" size="icon"
                                        class="absolute top-4 right-4 h-8 w-8" @click="remove(index)">
                                        <Trash2 class="h-4 w-4" />
                                    </Button>

                                    <FormField :name="`calons.${index}.peserta_id`">
                                        <FormItem>
                                            <FormLabel>Pilih Peserta</FormLabel>
                                            <VeeField :name="`calons.${index}.peserta_id`" v-slot="{ field }">
                                                <Select v-bind="field" @update:model-value="field.onChange">
                                                    <FormControl>
                                                        <SelectTrigger>
                                                    ...
                                                    <SelectContent>
                                                        <SelectGroup>
                                                            <SelectItem v-for="peserta in pesertaOptions"
                                                                :key="peserta.value" :value="peserta.value">
                                                                {{ peserta.label }}
                                                            </SelectItem>
                                                        </SelectGroup>
                                                    </SelectContent>
                                    ...
                                    <FormField :name="`calons.${index}.jabatan`">
                                        <FormItem>
                                            <FormLabel>Jabatan Calon</FormLabel>
                                            <VeeField :name="`calons.${index}.jabatan`" v-slot="{ field }">
                                                <Select v-bind="field" @update:model-value="field.onChange">
                                                    <FormControl>
                                                        <SelectTrigger>
                                                    ...
                                                    <SelectContent>
                                                        <SelectGroup>
                                                            <SelectItem v-for="jabatan in jabatanSelectOptions"
                                                                :key="jabatan.value" :value="jabatan.value">
                                                                {{ jabatan.label }}
                                                            </SelectItem>
                                                        </SelectGroup>
                                                    </SelectContent>
                                ...
                                <div class="flex flex-col sm:flex-row justify-between gap-4 pt-4">
                                    <Button type="button" variant="outline" @click="addCalonForm"
                                        :disabled="formInertia.processing">
                                        <Plus class="h-4 w-4 mr-2" /> Tambah Calon Lagi
                                    </Button>

                                    <div class="flex gap-2">
                                        <Button type="submit" :disabled="formInertia.processing">
                                            Simpan Semua Calon ({{ fields.length }})
                                        </Button>
                                        <Button type="button" variant="ghost" @click="$inertia.visit('/calon')">
                                            Batal
                                        </Button>
                                    </div>
                                </div>
