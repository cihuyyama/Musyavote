    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <Card className="rounded-lg border-none mt-2 w-full">
            <CardContent className="p-6 w-full">
                <div class="flex justify-center items-start min-h-[calc(100vh-56px-64px-20px-24px-56px-48px)] w-full">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                                <div class="flex ml-auto gap-2">
                                    <Button @click="showImportModal = true" variant="outline"
                                        class="border-gray-300 hover:bg-gray-50 text-gray-700">
                                        <Upload class="mr-2 h-4 w-4" /> Import Excel
                                    </Button>
                                    <Link href="/calon/create">
                                        <Button class="cursor-pointer bg-[#a81b2c] text-white hover:bg-[#8A1524]"
                                            variant="default">
                                            <Plus /> Tambah Calon
                                        </Button>
                                    </Link>
        ...
        <!-- Modal Import -->
        <ImportModal v-if="showImportModal" :show="showImportModal" :import-url="`calon/import`"
